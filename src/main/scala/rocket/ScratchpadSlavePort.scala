...
trait CanHaveScratchpad extends HasHellaCache with HasICacheFrontend {
  ...
  val fg = LazyModule(new TLFragmenter(fetchBytes, cacheBlockBytes, earlyAck=true)) // TODO: should these be moved outside the tile &
  val ww = LazyModule(new TLWidthWidget(xLenBytes))                                 //       generated by a new PeripheryBus method?
